...
async function updateWeatherInfo(city) {
    ...
    if (weatherData.cod !== 200) {
        forecastContainer.style.display = "none"; // Oculta el contenedor de pronósticos si la ciudad no existe
    ...
    forecastContainer.style.display = "flex" // Muestra la sección forecast si la ciudad existe
...
async function updateForecastsInfo(city) {
    ...
    if (!forecastsData.list || forecastsData.list.length === 0) {
        forecastContainer.style.display = "none" // Oculta los pronósticos si no hay datos
        return
    }
...
function updateForecastsItems(weatherData) {
    ...
    const dateResult = dateTaken.toLocaleDateString("es-ES", dateOption)
